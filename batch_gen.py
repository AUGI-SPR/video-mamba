...
class BatchGenerator(object):
    ...
    def read_data(self, vid_list_file):
        self.list_of_examples = sorted([f for f in os.listdir(vid_list_file) if f.endswith('.mp4')])
        ##file_ptr = open(vid_list_file, 'r')
        ##self.list_of_examples = file_ptr.read().split('\n')[:-1]
        ##file_ptr.close()
        self.gts = [os.path.join(self.gt_path, vid.split('.')[0] + '.txt') for vid in self.list_of_examples]
        self.features = [self.features_path + vid.split('.')[0] + '.pkl' for vid in self.list_of_examples]
        self.my_shuffle()
